...
def frameToBytes(frame):
    ...
    syndrome = bits @ H
    bits[np.all(syndrome == [1, 0, 0], axis=1), 0] ^= 1
    bits[np.all(syndrome == [0, 1, 0], axis=1), 1] ^= 1
    bits[np.all(syndrome == [0, 0, 1], axis=1), 2] ^= 1
    bits[np.all(syndrome == [1, 1, 0], axis=1), 3] ^= 1
    bits[np.all(syndrome == [0, 1, 1], axis=1), 4] ^= 1
    bits[np.all(syndrome == [1, 1, 1], axis=1), 5] ^= 1
    bits[np.all(syndrome == [1, 0, 1], axis=1), 6] ^= 1

    bits = bits[:, 3: 7]
    bits = bits.reshape(BYTE_PER_PAGE, 8)
    bits <<= BYTE_LEFT_SHIFT
